...
var opts = {
    ...
    // 上传之前
    beforeSend: function () {
        $("#uploader_button_icon").attr("class", "glyphicon glyphicon-repeat");
    },

    // 上传成功后
    success: function (result, status, xhr) {
        $("#uploader_image_url").val(result.data);
        $("#uploader_image_show").attr('src', result.data);
        $("#uploader_button_icon").attr("class", "glyphicon glyphicon-ok");
    },

    // 上传失败
    error: function (result, status, errorThrown) {
		alert(result.responseJSON.message)
		$("#uploader_button").attr("class", "btn btn-error btn-sm")
        $("#uploader_button_icon").attr("class", "glyphicon glyphicon-cloud");
    }
}

// 实例化插件
$('#uploader_file').fileUpload(opts);

// 启动按钮
document.getElementById("uploader_button").onclick = function() {
	document.getElementById('uploader_file').click();
}